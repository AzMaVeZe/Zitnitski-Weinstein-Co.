...
- **Capital gains: the user brings the indexation — no CPI table, ever.** (Owner-approved reversal, July 2026, of the earlier nominal-only simplification.) The engine never contains CPI data or computes indexation itself. Individual share CG (`sharesIndividualCG`) taxes the REAL gain: the user enters an already-indexed cost basis, and with purchase/sale dates the engine allocates the gain linearly across the §91 rate periods (form-1399 mechanics) — pre-1.1.2003 (המועד הקובע) at a user-set marginal rate (default 47%, `shcg_marginalPct`, field visible only when the purchase predates 1.1.2003), 1.1.2003–31.12.2011 at 20%/25%, from 1.1.2012 at 25%/30%; without dates the whole gain is treated as post-2012 and the UI shows the `shcg_dateNote` assumption warning. Individual RE CG (residential + commercial) accepts an **optional** user-entered indexed basis (`cg_res_indexedBasis`/`cg_com_indexedBasis`) that replaces the nominal adjusted basis (expenses still deducted); empty → nominal, and the gain display labels which mode ran. Crypto, §102, and the company CG sections remain nominal. The pre-1994 10% inflationary-amount rule is not modeled (disclosure only).
